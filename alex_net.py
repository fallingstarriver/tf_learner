...
def inference(images):
    ...
    with tf.name_scope('conv5') as scope:
        ...
        parameters += [kernel, biases]
        print_acctivation(conv5)

    pool5 = tf.nn.max_pool(conv5, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool5')

    print_acctivation(pool5)
...
def time_tensorflow_run(session,target,info_string):
    ...
    for i in range(num_batches+num_step_burn_in):
        ...
        total_duration_squred += duration*duration

    mn = total_duration / num_batches
    vr = total_duration_squred / num_batches - mn * mn
    sd = math.sqrt(abs(vr))
...
def run_benchmark():
    with tf.Graph().as_default():
        ...
        grand = tf.gradients(objective,parameters)
        time_tensorflow_run(sess,grand,"Forward-backward")
